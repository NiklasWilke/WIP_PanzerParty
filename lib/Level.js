...
class Level
{
	constructor(file)
	{
		...
		if (DEV) console.log("loading level "+level.name+" ["+this.id+"] ("+this.width+"x"+this.height+")");
		...
		for (var s=0; s<shapes.length; s++)
		{
			...
			//if (DEV) console.log("shape #"+s+" ["+tiles.length+"] "+min_x+","+min_y+" to "+max_x+","+max_y);
			for (var y=min_y; y<=max_y; y++)
			{
				...
				//if (DEV) console.log("checking Y"+y, row, " > X ", min_x+" to "+max_x);
				for (var x=min_x; x<=max_x; x++)
				{
					//if (DEV) console.log("checking ", x, y);
	...
	// {
		...
		// {
			...
			// if (DEV) console.log(pos, "->"+dir, [path.length]);
			...
			// {
				// if (DEV) console.log("> look around ("+pos.x+";"+pos.y+")");
				// for (var i=0; i<4; i++)
				// {
