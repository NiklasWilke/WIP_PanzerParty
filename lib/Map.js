...
class Map extends EventEmitter2
{
	constructor(level)
	{
		...
		if (DEV) console.log("ini map > ", level.file);
	...
	spawnPowerup()
	{
		...
		if (available_locations.length > 0)
		{
			...
			if (DEV) console.log("> spawn powerup ("+powerup.name+") ", pos);

			this.emit("powerup.spawned", powerup);
			return true;
		}
		else
		{
	...
	checkCollision(obj)
	{
		if (obj instanceof Bullet)
		{
			...
			{
				if (DEV) console.log("B:"+obj.id+" collision with ", tile);
		...
		else if (obj instanceof Bot)
		{
			...
			var center = this.getTileAtPos(bot.x, bot.y);
			if (center == null) return null;
			var collisions = [];
			...
			for (var x=Math.max(center.x-1, 0); x<=Math.min(center.x+1, this.width-1); x++)
			{
				for (var y=Math.max(center.y-1, 0); y<=Math.min(center.y+1, this.height-1); y++)
				{
					...
					var dist = 100;
					if (tile.x_abs <= bot.x && tile.x_abs + tile.size >= bot.x)
					{
						dist = Math.abs((tile.y_abs + tile.size / 2) - bot.y) - tile.size / 2 - bot.radius;
					}
					else if (tile.y_abs <= bot.y && tile.y_abs + tile.size >= bot.y)
					{
						dist = Math.abs((tile.x_abs + tile.size / 2) - bot.x) - tile.size / 2 - bot.radius;
					}
					else
					{
						var p = new Point(bot.x, bot.y);
						dist = Math.min(
							p.distanceTo(tile.x_abs, tile.y_abs),
							p.distanceTo(tile.x_abs + tile.size, tile.y_abs),
							p.distanceTo(tile.x_abs, tile.y_abs + tile.size),
