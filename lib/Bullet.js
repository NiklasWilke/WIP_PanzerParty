...
class Bullet extends EventEmitter2
{
	...
	// move by [distance] in [angle] direction
	move(distance)
	{
		this.x += Math.cos(toRadians(this.angle)) * distance;
		this.y += Math.sin(toRadians(this.angle)) * distance;
	}
	
	// move with [speed] in [angle] direction
	update()
	{
		this.move(1 * this.speed * 0.3);
	}
