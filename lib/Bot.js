const EventEmitter2 = require("eventemitter2").EventEmitter2;
const Color = require("./Color.js");
const Bullet = require("./Bullet.js");
const crypto = require("crypto");

function toRadians(angle)
{
	return angle * (Math.PI / 180);
}

class Bot extends EventEmitter2
{
    constructor(id, color)
    {    
        super();
        var self = this;
        
        this.id = crypto.createHash("md5").update(Math.round(Math.random()*89999+10000)+"").digest("hex");
        this.x = 50;
		this.y = 50;
		this.radius = 1.3;
        this.angle = 0;
        this.current_rotation = 0;
        this.current_rotation_tick = 0;
		this.speed = 1;
		this.base_speed = 0;//0.6;
		this.health = 500;
		this.hitbox_r = 1;
        this.color = new Color(100, 65, 50);
        
		this.max_bullets = 80; //10 waves * 8 bullets each
        this.bullets = 0;
        //shoots every x seconds
        this.attackspeed = 3;
        //delay between the waves
        this.attack_delay = 18;
        //amount of waves fired
        this.waves_amount = 3;

        this.rotation = 360/16 //this.waves_amount;

        this.loop_tick = 0;
        this.shoot_ticks = 0;

        this.behaviour = [];
        
        this.on("death", function(killer)
		{
			self.setSpeed(0);
        });
    }
        
        setAngle(a)
        {
            this.angle = a;
        }
        
        setSpeed(s)
        {
            this.speed = s;
        }

        distanceTo(obj)
        {
            return Math.sqrt(Math.pow(this.x - obj.x, 2) + Math.pow(this.y - obj.y, 2));
        }


        //circle shoot
        shoot(rotation)
        {
            var self;
            if (this.health <= 0 || this.bullets >= this.max_bullets) return false;
            
			var n = 8;
			for (var i=0; i<n; i++)
			{
				var bullet_id = crypto.createHash("md5").update(this.id+Math.round(Math.random()*89999+10000)).digest("hex");
				var bullet = new Bullet(bullet_id, this.x, this.y,rotation + i * (360 / n));
				bullet.move(this.radius);
				bullet.setMaxBounces(0);
				bullet.setColor(this.color);
				bullet.setOwner(this);
				bullet.setSpeed(0.7);
				
				bullet.on("despawn", function()
				{
					bullet.shooter.bullets --;
				});
				
				this.bullets ++;
				this.emit("bulletFired", bullet);
			}
        }

        registerHit(bullet)
        {
            bullet.despawn();
            this.health -= bullet.damage;
            
            if (this.health <= 0)
            {
                this.emit("death", bullet.shooter);
            }
            else
            {
                this.emit("hit", bullet.shooter);
            }
        }
        
		update()
		{
            if (this.health <= 0) return false;
            
			if (this.loop_tick <= 480) //8sec = 60fps * 16 = 960
			{
                this.loop_tick++;
                this.current_rotation_tick = 0;
				if (this.loop_tick % (60 * this.attackspeed) == 0) //every 1 sec
				{
                    this.shoot_ticks = 0;
                    //this.current_rotation = 0;
                    //SHOOT   
                    //this.current_rotation_tick = this.current_rotation;
                    this.shoot(this.current_rotation);
                    this.shoot_ticks ++;
                    console.log("1 Wave rotation:" + this.current_rotation)
                   
                }
                else if (this.shoot_ticks > 0 && this.shoot_ticks< ((this.waves_amount-1) * this.attack_delay))
                {
                    this.shoot_ticks++;
                    if (this.shoot_ticks % this.attack_delay == 0)
                    {
                        this.current_rotation = this.current_rotation + this.rotation;
                        this.current_rotation_tick = this.current_rotation;
                        this.shoot(this.current_rotation);   
                        console.log(this.shoot_ticks+ "Wave rotation:" + this.current_rotation)
                    }
                }
                this.x += Math.cos(toRadians(this.angle)) * this.speed * this.base_speed;
                this.y += Math.sin(toRadians(this.angle)) * this.speed * this.base_speed;
			}
            else
            {
                this.loop_tick = 0;
                this.x += Math.cos(toRadians(this.angle)) * this.speed * this.base_speed;
                this.y += Math.sin(toRadians(this.angle)) * this.speed * this.base_speed;
                
            }
            
        }

        toObject()
        {
            return {
                id: this.id,
                name: this.name,
                color: this.color,
                x: this.x,
                y: this.y,
                angle: this.angle,
                radius: this.radius
            };
        }

}

module.exports = Bot;



