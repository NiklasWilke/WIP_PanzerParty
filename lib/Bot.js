...
class Bot extends EventEmitter2
{
    ...
    {    
		...
		this.rotation_speed = 40; // degrees / sec
		this.move_duration = 3; // secs
        this.bullets = 0;
        //shoots every x seconds
        this.cooldown = 3; // secs
        //delay between the waves
        this.wave_delay = 1; // secs
        //amount of waves fired
        this.wave_amount = 3;
        this.wave_rotation = 360 / (8*3) * 2;
		...
		console.log (this.move_duration+" * 60 + "+this.wave_amount+" * ("+this.wave_delay+" + "+this.wave_rotation+" / "+this.rotation_speed+") = "+(this.move_duration * 60 + this.wave_amount * (this.wave_delay + this.wave_rotation / this.rotation_speed)));
    }
		...
		update()
		{
			...
			if (this.loop_tick <= this.move_duration * 60)
			{
				console.log("move");
				if (this.speed * this.base_speed <= this.max_speed)
				{
					this.x += Math.cos(toRadians(this.angle)) * this.speed * this.base_speed;
					this.y += Math.sin(toRadians(this.angle)) * this.speed * this.base_speed;
				}
				...
				{
					this.x += Math.cos(toRadians(this.angle)) * this.max_speed;
					this.y += Math.sin(toRadians(this.angle)) * this.max_speed;
				}
			}
			else if (this.loop_tick < this.move_duration * 60 + this.wave_amount * (this.wave_delay * 60 + this.wave_rotation / this.rotation_speed * 60))
			{
				var tick = Math.round(this.loop_tick - this.move_duration * 60);
				var wave = tick / (this.wave_delay * 60 + this.wave_rotation / this.rotation_speed * 60);
				tick = tick % (this.wave_delay * 60 + this.wave_rotation / this.rotation_speed * 60);
				
				console.log("W:"+wave+";T:"+this.loop_tick+"("+tick+")");
				
				if (tick < Math.round(this.wave_rotation / this.rotation_speed * 60))
				{
					this.rotation = (this.rotation + (this.wave_rotation / this.rotation_speed * 60)) % 360;
				}
				else if (tick == Math.round(this.wave_rotation / this.rotation_speed * 60))
				{
					console.log("shoot");
					this.shoot();
				}
