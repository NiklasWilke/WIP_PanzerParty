...
class GameEngine extends EventEmitter2
{
	...
	spawnBot()
	{
		...
		{
			console.log("B:"+bot.id+" fired B:"+bullet.id+" ("+bullet.type+") at ["+(Math.round(bullet.x*100)/100)+", "+(Math.round(bullet.y*100)/100)+"] "+(Math.round(bullet.angle*100)/100)+"°", bullet);
		...
		{
			// despawn tank
			for (var bt in self.bots)
			{
				if (self.bot[bt].id == bot.id)
				{
					self.bots.splice(bt, 1);
					break;
				}
			}
		});
		bot.on("death", function(killer)
		{
			...
			for (var bt in self.bots)
			{
				if (self.bots[bt].id == bot.id)
				{
					self.graveyard.push(bot.toObject());
					self.bots.splice(t, 1);
					break;
				}
			}
		});
		bot.on("hit", function()
		{
			player.socket.emit("hit", bot.health);
		});
	...
	checkCollision(a, b)
	{
		...
		if ((a instanceof Bullet && b instanceof Bot) || (b instanceof Bullet && a instanceof Bot))
		{
			...
			return (Math.abs(dx) <= bot.height / 2) && (Math.abs(dy) <= bot.width / 2);
		}
	}
	...
	updateLoop()
	{
		...
		for (var b in this.bullets)
		{
			...
			for (var bt in this.bots)
			{
				...
				{
					console.log("T:"+bot.id+" hit by B:"+bullet.id);
