...
class GameEngine extends EventEmitter2
{
	constructor(io)
	{
		if (DEV) console.log("ini GameEngine");
		super();
		...
		this.next_bot_spawn = null;
		//spawntime gets logged in the spawn function
		this.last_bot_spawn = null;
	...
	ini()
	{
		if (DEV) console.log("> initialising game");
		this.state = "ready";
	...
	start()
	{
		if (DEV) console.log("> starting game");
		this.state = "running";
		this.start_time = Date.now();
		if (SPAWN_BOTS) this.next_bot_spawn = Date.now() + 60 * 1000;
	...
	stop()
	{
		if (DEV) console.log("> stopping game");
		this.tanks = [];
		this.bots = [];
		this.graveyard = [];
		this.bullets = [];
	...
	updateTimer()
	{
		...
		var time = Date.now();
		this.timer_time = Math.floor((time - this.start_time) / 1000);
		this.sockets.emit("updateTimer", this.timer_time);

		if (this.next_bot_spawn != null && time >= this.next_bot_spawn)
		{
			this.next_bot_spawn = null;
			var bot = this.spawnBot();
			bot.on("death", function()
			{
				self.next_bot_spawn = Date.now() + 60 * 1000;
			});
		}
	}
	...
	setLevel(level)
	{
		...
		{
			if (DEV) console.log("update powerups!");
	...
	spawnTank(player)
	{
		...
		{
			if (DEV) console.log("T:"+tank.id+" fired B:"+bullet.id+" ("+bullet.type+") at ["+(Math.round(bullet.x*100)/100)+", "+(Math.round(bullet.y*100)/100)+"] "+(Math.round(bullet.angle*100)/100)+"°");
		...
		{
			player.kills += (victim instanceof Tank ? 1 : 3);
			self.emit("updateScoreboard");
		});
		...
		{
			player.socket.emit("updateHealth", hp);
		});
		...
		{
			if (DEV) console.log("> T:"+tank.id+" collected ", powerup.name);
		...
		{
			if (DEV) console.log("> T:"+tank.id+" activated ", powerup.name);
		...
		if (DEV) console.log("> spawned T:"+tank.id+" at ["+tank.x+", "+tank.y+"]");
	}
	...
	spawnBot()
	{
		...
		{
			//if (DEV) console.log("BOT:"+bot.id+" fired B:"+bullet.id+" ("+bullet.type+") at ["+(Math.round(bullet.x*100)/100)+", "+(Math.round(bullet.y*100)/100)+"] "+(Math.round(bullet.angle*100)/100)+"°");
		...
		{
			if (DEV) console.log("BOT:"+bot.id+" killed by T:"+killer.id);
		...
		self.last_bot_spawn = Date.now();
		self.bots.push(bot);

		return bot;

		if (DEV) console.log("> spawned B:"+bot.id+" at ["+bot.x+", "+bot.y+"]");
    }
	...
	checkCollision(a, b)
	{
		...
		if ((a instanceof Bullet && b instanceof Tank) || (b instanceof Bullet && a instanceof Tank))
		{
			...
			//if (DEV) console.log("check colision > T["+tank.x+", "+tank.y+"] > B["+bullet.x+", "+bullet.y+"] >> ["+temp.x+", "+temp.y+"] d:"+dist+" | "+Math.sqrt(Math.pow(temp.x-tank.x, 2) + Math.pow(temp.y-tank.y, 2)));
	...
	bullet_helper(bullet, distance)
	{
		...
		if (tile == null)
		{
			if (DEV) console.log("B:"+bullet.id+" ("+bullet.type+") out of bounds: ["+bullet.x+", "+bullet.y+"]");
		...
		// if (DEV) console.log("> updating B:"+bullet.id+" ("+bullet.type+")");
	...
	{
		this.updateTimer();
		//if (DEV) console.log("Time" + this.game_time);
		...
		for (var b in this.bullets)
		{
			...
			for (var t in this.tanks)
			{
				...
				{
					if (DEV) console.log("T:"+tank.id+" hit by B:"+bullet.id);
			...
			for (var bt in this.bots)
			{
				...
				{
					if (DEV) console.log("BOT:"+bot.id+" hit by B:"+bullet.id);
