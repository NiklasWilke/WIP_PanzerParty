...
class GameEngine extends EventEmitter2
{
	...
	stop()
	{
		...
		this.sockets.emit("update", {graveyard: [], bullets: [], tanks: [], powerups: []});
	}
	...
	spawnBot()
	{
		...
		{
			console.log("T:"+bot.id+" fired B:"+bullet.id+" ("+bullet.type+") at ["+(Math.round(bullet.x*100)/100)+", "+(Math.round(bullet.y*100)/100)+"] "+(Math.round(bullet.angle*100)/100)+"°");
