...
class GameEngine extends EventEmitter2
{
	...
	bullet_helper(bullet, distance)
	{
		...
		console.log("bullet at ("+(Math.round(bullet.x/tile.size*1000)/1000)+"|"+(Math.round(bullet.y/tile.size*1000)/1000)+") =>", [tile.x, tile.y]);
		
		var s = bullet.s;
		var b = bullet.b;
		
		
		console.log("moving with ["+(Math.round(bullet.vector[0]*10000)/10000)+", "+(Math.round(bullet.vector[1]*10000)/10000)+"] * "+distance);
		...
		console.log("> top    "+top, [rel_top]);
		console.log("> right  "+right, [rel_right]);
		console.log("> bottom "+bottom, [rel_bottom]);
		console.log("> left   "+left, [rel_left]);
		...
		if (x < y) // check X-axis
		{
			if (bullet.vector[0] < 0) // check left border
			{
				border = "LEFT";
				dir = "RIGHT";
				X = tile.x_abs - tile.size * Math.floor(Math.abs(left) / 2); // bullet.x - left * bullet.vector[0];
				console.log(">> LEFT BORDER", X/tile.size);
			}
			else // check left border
			{
				border = "RIGHT";
				dir = "LEFT";
				X = tile.x_abs + tile.size + tile.size * Math.floor(Math.abs(right) / 2); // bullet.x + right * bullet.vector[0];
				console.log(">> RIGHT BORDER"+X/tile.size);
			}
			Y = X * s + b;
		}
		else if (y < x) // check Y-axis
		{
			if (bullet.vector[1] < 0) // check top border
			{
				...
				border = "TOP";
				dir = "DOWN";
				Y = tile.y_abs - tile.size * Math.floor(Math.abs(top) / 2); // bullet.y - top * bullet.vector[1];
				console.log(">> TOP BORDER", Y/tile.size);
			}
			else // check bottom border
			{
				...
				border = "BOTTOM";
				dir = "UP";
				Y = tile.y_abs + tile.size + tile.size * Math.floor(Math.abs(bottom) / 2); // bullet.y + bottom * bullet.vector[1];
				console.log(">> BOTTOM BORDER", Y/tile.size);
			}
			X = (Y - b) / s;
		}
		else // hmm
		...
		//console.log(">> ", [X, Y], distance_to_border, "> move by "+step);
		console.log("> next tile", [next_tile.x, next_tile.y], "in "+distance_to_border, " entering at ("+(Math.round(X/tile.size*1000)/1000)+"|"+(Math.round(Y/tile.size*1000)/1000)+")");
		
		//console.log("");
		if (distance_to_border <= distance)
		{
			console.log("> passing "+border+" border at ("+(Math.round(X/tile.size*1000)/1000)+"|"+(Math.round(Y/tile.size*1000)/1000)+")");
