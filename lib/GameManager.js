...
class GameManager extends EventEmitter2
{
	constructor(io)
	{
		if (DEV) console.log("ini GameManager");
		super();
		...
		{
			if (DEV) console.log(player.name, " joined");
			self.updateAvailableColors();
		});
		this.on("playerExited", function(player)
		{
			if (DEV) console.log(player.name, " left");
			self.updateAvailableColors();
		});
	...
	iniClient(socket)
	{
		...
		{
			if (DEV) console.log("set level >", level_id);
		...
		{
			if (DEV) console.log("socket -> start game");
			self.startGame();
		});
		...
		{
			if (DEV) console.log("saving level '"+name+"' to '"+file_name+"'");
			...
			fs.writeFile("./data/maps/"+file_name, json, "utf8", function(res)
			{
				...
				for (var l in self.levels)
				{
					if (self.levels[l].id == level.id)
					{
						...
						if (DEV) console.log("overwritten");
						break;
					}
				}
	...
	updateScoreboard()
	{
		...
		if (list[0] && list[0].kills >= this.kill_goal)
		{
			this.game.end();
