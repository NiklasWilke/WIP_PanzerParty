...
class Tank extends EventEmitter2
{
	constructor(player)
	{
		...
		//weapontype for custom tank pipes
		this.weapontype = "test";
		this.powerup = null;
		this.effects = [];
	...
	{
		bullet.despawn();
		this.health -= Math.max(bullet.damage-this.getEffect("bonus_health"), 0);
	...
	activatePowerup()
	{
		if (this.powerup)
		{
			if (this.powerup.type == "effect")
			{
				this.emit("activatePowerup", this.powerup);
				this.powerup = null;
				return true;
			}
			else if (this.powerup.type == "weapon")
			{
