...
class Tank extends EventEmitter2
{
	...
	update()
	{
		...
		if ((this.speed * this.base_speed * this.getEffect("movement_speed") <= this.max_speed))
		{
		...
		if (this.powerup)
		{
			...
			else if (this.powerup.weapon)
			{
				for (var i=0; i<this.powerup.weapon.bullets; i++)
				{
					...
					bullet.setColor(this.color);
					bullet.setOwner(this);
					if (typeof this.powerup.weapon.speed !== "undefined" && this.powerup.weapon.speed <= bullet.max_speed) bullet.setSpeed(this.powerup.weapon.speed);
